...
```bash
sam local invoke getAllIngressesFunction --event events/event-get-all-ingresses.json
```
...
Resources:
  ...
  getAllItemsFunction:
    ...
    Properties:
    Handler: src/handlers/get-all-ingresses.getAllIngressesHandler
      Runtime: nodejs18.x
      DeadLetterQueue:
        Type: SQS 
...
```bash
aws cloudformation delete-stack --stack-name aws-serverless-api-prototype
```
